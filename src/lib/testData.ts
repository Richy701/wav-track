...
export const insertTestData = async (): Promise<void> => {
  try {
    ...
    const projects = projectData.map((project) => {
      ...
      const last_modified = new Date(new Date(created_at).getTime() + Math.random() * 1000 * 60 * 60 * 24 * 14).toISOString(); // 0-14 days after creation
    ...
    for (let i = 0; i < projects.length; i += batchSize) {
      try {
        ...
        console.log(`Inserting batch ${i / batchSize + 1} of ${Math.ceil(projects.length / batchSize)}`);
  ...
  try {
    ...
    for (let i = 0; i < activities.length; i += batchSize) {
      try {
        ...
        console.log(`Inserting beat activities batch ${i / batchSize + 1} of ${Math.ceil(activities.length / batchSize)}`);
        ...
        if (error) {
          ...
          console.log(`Successfully inserted beat activities batch ${i / batchSize + 1}:`, data?.length, 'activities');
          successCount += data?.length || 0;
        }
      } catch (batchError) {
    ...
    console.log(`Test beat activities insertion complete! Successfully inserted ${successCount} activities`);
  } catch (error) {
  ...
  try {
    ...
    for (let i = 0; i < sessions.length; i += batchSize) {
      try {
        ...
        console.log(`Inserting sessions batch ${i / batchSize + 1} of ${Math.ceil(sessions.length / batchSize)}`);
        ...
        if (error) {
          ...
          console.log(`Successfully inserted sessions batch ${i / batchSize + 1}:`, data?.length, 'sessions');
          successCount += data?.length || 0;
        }
      } catch (batchError) {
