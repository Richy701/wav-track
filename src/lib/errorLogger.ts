...
export const errorLogger = {
  ...
  /**
   * Initialize error logging and monitoring
   */
  init() {
    this.setupGlobalErrorHandlers()
    this.monitorSupabaseErrors()
  },

  /**
   * Monitor Supabase errors and log them
   */
      ...
      try {
        ...
        if (isSupabaseRequest && !response.ok) {
          try {
            ...
            errorLogger.logError('Supabase API Error', `${response.status}: ${response.statusText}`, {
        ...
        if (isSupabaseRequest) {
          errorLogger.logError('Supabase Network Error', error instanceof Error ? error.message : String(error), {
            url,
          })
        }
        throw error
  ...
  setupGlobalErrorHandlers() {
    ...
    window.addEventListener('error', event => {
      errorLogger.logError('Unhandled Error', event.message, {
