...
const loadAudioElement = async (url: string): Promise<HTMLAudioElement> => {
  ...
  audio.addEventListener('loadstart', () => console.log(`Audio loading started for ${url}`), { once: true });
  audio.addEventListener('durationchange', () => console.log(`Audio duration available for ${url}: ${audio.duration}s`), { once: true });
  audio.addEventListener('loadedmetadata', () => console.log(`Audio metadata loaded for ${url}`), { once: true });
  ...
  if (ctx) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`HTTP error loading audio! status: ${response.status} (${response.statusText}), Content-Type: ${contentType}, Content-Length: ${contentLength}`);
      ...
      try {
        ...
        console.warn(`Web Audio API decoding failed for ${url}, falling back to HTML5 Audio:`, decodeError);
        throw decodeError;
      }
    } catch (error) {
