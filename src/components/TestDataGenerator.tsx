...
import { insertAllTestData, insertTestData, insertTestBeatActivities, insertTestSessions } from '@/lib/testData';
import { inspectProjectsTable, checkExistingData } from '@/lib/debugSchema';
import { AlertTriangle, Bug, Database, Loader2, Play, Plus, RefreshCw, Trash2, Wrench } from "lucide-react";
    ...
    <div className="rounded-lg border p-4 shadow-sm">
      ...
      <Tabs defaultValue="generate">
        <TabsList className="w-full mb-4">
          <TabsTrigger value="generate" className="flex-1">Generate Data</TabsTrigger>
          <TabsTrigger value="debug" className="flex-1">Debug Tools</TabsTrigger>
        </TabsList>
              ...
              {errors.length === 0 ? (
                ...
                <div className="max-h-[200px] overflow-y-auto border rounded-md text-xs">
                  {errors.map((error, index) => (
                    <div 
                      key={index} 
                      className="p-2 border-b last:border-0 hover:bg-muted/50"
                    >
