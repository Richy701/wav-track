...
const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid w-full auto-rows-[18rem] grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4",
        className,
      )}
    >
      {children}
...
const BentoCard = ({
  ...
  >
    ...
    <div className="relative z-10 flex h-full flex-col">
      ...
      <div className="flex transform-gpu flex-col gap-1 p-4 sm:p-6 transition-all duration-300 group-hover:-translate-y-2">
        <div className={cn(
          "h-10 w-10 sm:h-12 sm:w-12 rounded-xl p-2 sm:p-2.5 transform-gpu transition-all duration-300 ease-in-out group-hover:scale-95",
        ...
        <h3 className="text-lg sm:text-xl font-semibold text-foreground dark:text-foreground/90">
          {name}
        </h3>
        <p className="max-w-lg text-sm sm:text-base text-muted-foreground dark:text-muted-foreground/80">{description}</p>
      </div>

      {/* CTA button */}
      <div className="mt-auto p-3 sm:p-4 opacity-0 transition-all duration-300 group-hover:opacity-100">
        <Button 
          variant="ghost" 
          asChild 
          size="sm" 
          className="relative hover:text-purple-500 dark:hover:text-purple-400 hover:bg-purple-500/10 dark:hover:bg-purple-500/5 text-sm sm:text-base"
        >
          <a href={href}>
            {cta}
